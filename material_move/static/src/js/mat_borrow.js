...
odoo.define('material_move.mat_borrow', function (require) {
"use strict";
// web odoo dependecies
var AbstractAction = require('web.AbstractAction');
var core = require('web.core');
var Session = require('web.session');
var canvas = require('material_move.canvas');
var _t = core._t;
var QWeb = core.qweb;
    ...
    // events in html elements
     events: {
        "click .manual": function(){
            ...
            this.do_action("material_move.binhex_op_type_select");
        },
    ...
    _onBarcodeScanned: function(barcode) {
        ...
        this._rpc({
                ...
                if (res.length > 0 && res[0] == 'p'){
                        ...
                        self.do_action("material_move.binhex_op_type_select");
                }
                else
...
var MatOrder = AbstractAction.extend({
    events: {
        ...
        "click #origPlace": function(e) {
            e.preventDefault();
            if (this.location){
                this.do_action('material_move.binhex_location_kanban_action',{
                    additional_context: {
                        type: "orig",
                    },
                ...
                this.do_action('material_move.binhex_warehouse_kanban_action',{
                    additional_context: {
                        type: "orig",
                    },
                    clear_breadcrumbs: true
        ...
        "click .partner": function(e) {
            e.preventDefault();
            this.do_action('material_move.binhex_respon_kanban_action',{
                additional_context: {
                },
                clear_breadcrumbs: true
            });
        ...
        "click #add": function(e) {
            ...
            if (Session.orig)
                this._rpc({
                    ...
                    self.do_action('material_move.binhex_product_kanban_action',{
                        additional_context: {
                            domain: domain,
                        },
                        clear_breadcrumbs: true,
                ...
                this.do_action('material_move.binhex_product_kanban_action',{
                        additional_context: {
                            domain: domain,
                        },
                        clear_breadcrumbs: true,
        ...
        "click .exit": function(e) {
            ...
            if (Session.next_action){
                ...
                this.do_action('material_move.binhex_material_borrow',{
                    additional_context: {
                    },
                    clear_breadcrumbs: true
                });
        ...
        "click .start": function(e) {
            ...
            this._rpc({
                ...
                if (res.length){
                    ...
                    self.do_action('material_move.binhex_end_screen');
                    return;
                }
                $(".modal").modal('show')
            });
    ...
    start: async function () {
        ...
        var self = this;
        self.getSession().user_has_group('material_move.group_use_locations').then(function(has_group){
