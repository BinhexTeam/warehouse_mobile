...
class Warehouse(models.Model):
    ...
    def check_barcode_purchases(self, barcode):
        ...
        w = []
        if (len(l) > 0 and not self.env.user.has_group("material_move.group_use_locations")):
    ...
    def create_purchase(self, products, supplier, dest, validate, ref, rec_date, respon=False, sign=False):
        ...
        if self.env.user.has_group("material_move.group_use_locations"):
    ...
    def warehouse_ops_views(self, operation):
        ...
        return {
            ...
            'views': [[self.env.ref("inventory_log.binhex_stock_picking_kanban").id, 'kanban']],
            'target': 'fullscreen',
            'domain': domain,
        }
